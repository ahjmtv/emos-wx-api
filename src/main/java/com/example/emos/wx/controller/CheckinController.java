...
public class CheckinController {
    ...
    public R validCanCheckIn(@RequestHeader("token") String token) {
        System.out.println("--------------->/validCanCheckIn");
    ...
    public R createFaceModel(@RequestParam("photo") MultipartFile file, @RequestHeader("token") String token) {
        System.out.println("-------------->/createFaceModel");
        if (file == null) {
            return R.error("没有上传文件");
        }
        int userId = jwtUtil.getUserId(token);
        ...
        if (!fileName.endsWith(".jpg")) {
            ...
            try {
                ...
                System.out.println("---------------------->checkinService.createFaceModel(userId,path);");
                return R.ok("人脸建模成功");
            } catch (IOException e) {
                // 写入日志
                log.error(e.getMessage(), e);
    ...
    public R searchTodayCheckin(@RequestHeader("token") String token) {
        ...
        // 判断startTime是否在用户入职之前
        DateTime hiredate = DateUtil.parse(userService.searchUserHiredate(userId));
        // --为了测试而设置今天日期为2022/11/20
        DateTime today = DateUtil.parseDateTime("2022-11-20 10:10:10");
        DateTime startDate = DateUtil.beginOfWeek(today);
